...
export const entities: Writable<Map<string, Map<string, Entity>>> = writable(
    new Map(),
);

export const relations: Writable<Map<string, Map<string, string>>> = writable(
    new Map(),
);
...
function getLabel(resource: string): string {
    for (const [label, ent] of get(entities).entries()) {
        if (ent.has(resource)) {
            return label;
        }
    }
...
export function relate(source: string, target: string, relation: string): void {
    entities.update((relations) => {
        const resources = entities.get(label);
        const sourceRes = resources?.get(source);
        const targetRes = resources?.get(target);

        if (sourceRes && targetRes) {
            targetRes.count += sourceRes.count;
            if (sourceRes.name.length > targetRes.name.length) {
                targetRes.name = sourceRes.name;
            }
