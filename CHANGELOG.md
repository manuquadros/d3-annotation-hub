...
- Opening an article for annotation is faster: the referenced entities' names and synonyms are now loaded in a single query instead of two per entity, so an article citing many distinct entities no longer issues hundreds of extra database round-trips (an article referencing 300 entities dropped from ~90 ms to ~7 ms per load in a local benchmark).
